...
namespace SterlingSwitch.CustomControls.HorizontalList
{
    public class HorizontalList:Grid
    {
        ...
        public static readonly BindableProperty ItemsSourceProperty =
            BindableProperty.Create("ItemsSource", typeof(IEnumerable<object>), typeof(HorizontalList), default(IEnumerable<object>), BindingMode.TwoWay, propertyChanged: ItemsSourceChanged);
        ...
        private static void ItemsSourceChanged(BindableObject bindable, object oldValue, object newValue)
        {
          ...
          //  itemsLayout.SelectedItem = itemsLayout.ItemsSource.LastOrDefault();
          //  ScrollToView(itemsLayout);
        }
        ...
        private static async void ScrollToView(HorizontalList itemsView)
        {
            try
            {
                var c = itemsView.SelectedItem;
                var selectedView = itemsView.GetItemView(c);
                var view = itemsView._itemsStackLayout.Children.Where(x => x.BindingContext == selectedView.BindingContext).SingleOrDefault();
                //var view = itemsView._itemsStackLayout.Children[index];
                ...
                if (view != null)
                {
                    ...
                    await itemsView._scrollView.ScrollToAsync(x, y, true);
                }
                //await itemsView._scrollView.ScrollToAsync(view, ScrollToPosition.MakeVisible, true);
            }
            catch (Exception ex)
            {
                ;
